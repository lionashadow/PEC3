...
# Para que sea más facil interpretar las relaciones entre las variables vamos a crear una visualización de las interacciones en contexto de clasificación Normal-Anormal y Normal-Hernia-Espondilolistesis.
...
De todos los coeficientes de correlación de Pearson entre dos variables distintas, el que tiene la cifra más elevada en valor absoluto es el de la incidencia pélvica (pelvic_incidence) con la pendiente sacral (sacral_slope), con un coeficiente de 0.8149600. Por lo tanto, sería lógico analizar estas dos variables, que son las que tienen un mayor coeficiente de correlación, para la regresión. No obstante, según la bibliografía, la incidencia pélvica (PI) se calcula mediante la suma de la pendiente sacral (SS) más la inclinación pélvica (PT), e influye en las variables de medida angulares próximas. Véase su descripción y definición indicada en el artículo de J.C.Le Huec et al. (2011): 

<https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3175921/>

Por lo tanto, no tendría mucho sentido analizar la relación entre estas dos variables, ya que son dos variables que se sabe que están relacionadas desde un principio. Y lo mismo pasaría con la relación entre la incidencia pélvica con otras de las variables métricas. Es por eso que decidiremos comprobar la relación entre dos variables diferentes, pero que conserven un coeficiente de correlación elevado, como el ángulo de lordosis lumbar (lumbar_lordosis_angle) con la pendiente sacral (sacral_slope), que tiene un coeficiente de correlación distintivo de 0.59838689.

Así, en este ejercicio queremos comprobar si es cierto que el ángulo de lordosis lumbar es una variable que se correlaciona con la pendiente sacral. Al tener un coeficiente de correlación no muy elevado (no es muy próximo a 1), nuestra hipótesis es que es probable que no se cumpla esta relación. No obstante, en caso de darse, nuestra hipótesis sería estas dos variables tienen una relación positiva.
...
Con este gráfico verificamos que existen algunas anotaciones de nuestra muestra (outliers) que se alejan del comportamiento de la mayoría (es decir, que se alejan de la línea centrada al cero). Aun así, éstas parecen no ser significativas debido a que son pocas. 
...
Finalmente, creemos que el modelo estudiado parece válido y que por lo tanto parece no existir una relación significativa entre las dos variables de estudio. Representaremos el modelo final en el siguiente gráfico. Véase cómo existe parte de los datos de nuestra muestra que sigue un mismo comportamiento lineal ascendente para las dos variables, pero la nube de puntos que se aleja de este comportamiento es demasiado grande como para definir una relación lineal significativa.

```{r}
plot(ortho_df$sacral_slope,ortho_df$lumbar_lordosis_angle, main="Modelo de regresión lineal", xlab="Pendiente sacral", ylab= "Ángulo de lordosis lumbar")
abline(ortho_lm)
```

Como conclusión podemos decir que, como el coeficiente de determinación no es muy elevado, las dos variables analizadas (ángulo de lordosis lumbar y pendiente sacral) no parecen tener mucha relación en cuanto a su comportamiento. Esta falta de significación podría darse por la gran variabilidad en los elementos de la muestra, pero también porque miden elementos distintos a nivel anatómico pélvico.
